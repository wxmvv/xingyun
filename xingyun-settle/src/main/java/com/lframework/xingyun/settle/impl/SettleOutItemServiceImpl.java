...
public class SettleOutItemServiceImpl extends BaseMpServiceImpl<SettleOutItemMapper, SettleOutItem>
  ...
  @Override
  public PageResult<SettleOutItem> query(Integer pageIndex, Integer pageSize,
      QuerySettleOutItemVo vo) {
  ...
  @Override
  public PageResult<SettleOutItem> selector(Integer pageIndex, Integer pageSize,
      SettleOutItemSelectorVo vo) {
  ...
  @OpLog(type = SettleOpLogType.class, name = "删除支出项目，ID：{}", params = "#id")
  ...
  @OpLog(type = SettleOpLogType.class, name = "新增支出项目，ID：{}, 编号：{}", params = {"#id",
      "#code"})
  @Transactional(rollbackFor = Exception.class)
  @Override
  public String create(CreateSettleOutItemVo vo) {
    ...
    Wrapper<SettleOutItem> checkWrapper = Wrappers.lambdaQuery(SettleOutItem.class)
        .eq(SettleOutItem::getCode, vo.getCode()).eq(SettleOutItem::getAvailable, Boolean.TRUE);
  ...
  @OpLog(type = SettleOpLogType.class, name = "修改支出项目，ID：{}, 编号：{}", params = {"#id",
      "#code"})
  @Transactional(rollbackFor = Exception.class)
  @Override
  public void update(UpdateSettleOutItemVo vo) {
    ...
    Wrapper<SettleOutItem> checkWrapper = Wrappers.lambdaQuery(SettleOutItem.class)
        .eq(SettleOutItem::getCode, vo.getCode()).eq(SettleOutItem::getAvailable, Boolean.TRUE)
        .ne(SettleOutItem::getId, vo.getId());
    if (getBaseMapper().selectCount(checkWrapper) > 0) {
      throw new DefaultClientException("编号重复，请重新输入！");
    }
    ...
    LambdaUpdateWrapper<SettleOutItem> updateWrapper = Wrappers.lambdaUpdate(SettleOutItem.class)
        ...
        .set(SettleOutItem::getDescription,
            StringUtil.isBlank(vo.getDescription()) ? StringPool.EMPTY_STR : vo.getDescription())
